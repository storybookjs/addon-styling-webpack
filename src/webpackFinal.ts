...
export function webpackFinal(config: WebpackConfig, { rules, plugins }: AddonStylingOptions = {}) {
  logger.info(
    `=> [${picocolors.green('@storybook/addon-styling-webpack')}] Applying custom Storybook webpack configuration styling.`
  );
  if (plugins && plugins.length) {
    logger.info(
      `=> [${picocolors.green('@storybook/addon-styling-webpack')}] Adding given plugins to Storybook webpack configuration.`
    );
